...
function doRectanglesOverlap(rect1, rect2) {
  if (rect2.top < rect1.height + rect1.top && rect2.left < rect1.width + rect1.left) {
    return true;
  }
  return false;
}
...
function findFirstSingleChar(str) {
  ...
  return array.find((element) => array.filter((el) => el === element).length === 1) || null;
}
...
function isCreditCardNumber(ccn) {
  const check = Number(ccn.toString().slice(-1));
  const sum = ccn.toString().slice(0, -1).split('').reverse()
    .map((el) => Number(el))
    .map((el, ind) => {
      if (ind % 2) {
        return el;
      ...
      const res = el * 2;
      return res <= 9 ? res : res.toString().split('').map((elem) => Number(elem)).reduce((acc, element) => acc + element);
    })
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (open.includes(str[i])) {
      ...
      if (temp !== close.indexOf(str[i])) return false;
    }
  }
